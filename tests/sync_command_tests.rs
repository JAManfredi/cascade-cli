...
macro_rules! skip_without_ca {
    () => {
        if !ca_binary_exists() {
            eprintln!("Skipping test: ca binary not found - run 'cargo build --release' first");
            return;
        }
    };
}
...
fn test_sync_with_upstream_changes() {
    ...
    // Get pre-sync state
    let pre_sync_commits = get_current_branch_commits(repo_path);
    
    // Run sync
    let (success, stdout, stderr) = run_ca_command(&["sync"], repo_path).unwrap();
    assert!(success, "Sync should succeed with upstream changes: {}", stderr);
    
    // Should create new version branches
    let post_sync_branch = get_current_branch(repo_path);
    assert!(post_sync_branch.contains("-v2"), "Should create version branch after sync");
    
    // New branch should have the upstream changes
    let post_sync_commits = get_current_branch_commits(repo_path);
    assert!(post_sync_commits.iter().any(|c| c.contains("Upstream change")), 
...
fn test_sync_preserves_original_branches() {
    ...
    for (branch, original) in original_commits {
        ...
        assert_eq!(original, current, 
                   "Branch {} should have unchanged commits after sync", branch);
    }
}
...
fn test_sync_multiple_stacks() {
    ...
    assert!(all_branches.contains(&stack1_branch), "Stack 1 branches should exist");
    assert!(all_branches.contains(&stack2_branch), "Stack 2 branches should exist");
}