...
fn create_branch_with_commit(repo_path: &std::path::Path, branch_name: &str, file_name: &str) {
    ...
    std::fs::write(repo_path.join(file_name), format!("Content for {}", branch_name)).unwrap();
...
fn test_git_cherry_pick_preserves_source_branch() {
    ...
    let commit_hash = Command::new("git")
        ...
        .output()
        .unwrap();
    let commit_to_pick = String::from_utf8_lossy(&commit_hash.stdout).trim().to_string();
    
    // Create target branch and cherry-pick
    Command::new("git")
        .args(["checkout", "-b", "target", "master"])
        .current_dir(repo_path)
        .output()
        .unwrap();
...
fn test_rebase_creates_new_branches_not_modifying_original() {
    ...
    std::fs::write(repo_path.join("master.txt"), "Master update").unwrap();
    Command::new("git")
        .args(["add", "."])
        .current_dir(repo_path)
        .output()
    ...
    Command::new("git")
        .args(["checkout", "-b", "feature-v2", "master"])
        .current_dir(repo_path)
        .output()
        .unwrap();
...
fn test_multiple_version_branches_independent() {
    ...
    Command::new("git")
        .args(["checkout", "-b", "feature-v3", "master"])
        .current_dir(repo_path)
        .output()
        .unwrap();
    ...
    assert_ne!(original_commits[0], v2_commits[0], "v2 should have different commit hash");
    assert_ne!(original_commits[0], v3_commits[0], "v3 should have different commit hash");
    assert_ne!(v2_commits[0], v3_commits[0], "v2 and v3 should have different commit hashes");
...
fn test_branch_versioning_naming_patterns() {
    ...
    for (original, expected_v2) in patterns {
        ...
        let v2_name = if original.contains("-v") && original.split("-v").last().unwrap().parse::<u32>().is_ok() {
            format!("{}-v2", original)
        } else {
            format!("{}-v2", original)
        };
