...
fn test_rebase_conflict_detection() {
    ...
    git_repo.checkout_branch("master").unwrap();
    ...
    assert!(!is_in_rebase_state(repo_path), "Should not be in rebase state after abort");
}
...
fn test_interrupted_rebase_recovery() {
    ...
    git_repo.checkout_branch("master").unwrap();
    std::fs::write(repo_path.join("master.txt"), "Master content\n").unwrap();
    Command::new("git")
        .args(["add", "."])
        .current_dir(&repo_path)
        .output()
...
fn test_rebase_continue_after_conflict_resolution() {
    ...
    std::fs::write(repo_path.join("file.txt"), "Feature line 1\nFeature line 2\n").unwrap();
    Command::new("git")
        .args(["add", "."])
        .current_dir(&repo_path)
        .output()
    ...
    // Create non-conflicting change on master
    git_repo.checkout_branch("master").unwrap();
    ...
    Command::new("git")
        .args(["commit", "-m", "Master other change"])
        .current_dir(&repo_path)
        .output()
        .unwrap();
    ...
    assert!(output.status.success(), "Rebase should succeed without conflicts");
    assert!(!is_in_rebase_state(repo_path), "Should not be in rebase state");
}
...
fn test_cascade_handles_dirty_working_directory() {
    ...
    assert!(status.contains("uncommitted.txt"), "Should show uncommitted file");
}
...
fn test_rebase_preserves_commit_metadata() {
    ...
    std::fs::write(repo_path.join("authored.txt"), "Content by specific author\n").unwrap();
    Command::new("git")
        .args(["add", "."])
        .current_dir(&repo_path)
        .output()
    ...
    Command::new("git")
        .args([
            "-c", "user.name=Special Author",
            "-c", "user.email=special@example.com",
            "commit", "-m", "Authored commit"
        ])
        .current_dir(&repo_path)
        .output()
        .unwrap();
    ...
    let original_author = String::from_utf8_lossy(&original_info.stdout).trim().to_string();
    ...
    let rebased_author = String::from_utf8_lossy(&rebased_info.stdout).trim().to_string();
...
"#;
    ...
    assert!(content.contains("<<<<<<<"), "Should contain conflict start marker");
    assert!(content.contains("======="), "Should contain conflict separator");
    assert!(content.contains(">>>>>>>"), "Should contain conflict end marker");
}
...
fn test_stash_pop_conflicts() {
    ...
    assert!(!pop_output.status.success(), "Stash pop should fail with conflict");
    ...
    assert!(!stash_output.is_empty(), "Stash should still exist after failed pop");
}
