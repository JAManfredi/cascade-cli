...
impl BitbucketIntegration {
    ...
    fn generate_stack_hierarchy(
        ...
        // Add stack overview
        hierarchy.push_str(&format!("**Stack:** `{}`\n", stack.name));
        hierarchy.push_str(&format!("**Base Branch:** `{}`\n", stack.base_branch));
        hierarchy.push_str(&format!("**Total Entries:** {}\n\n", stack.entries.len()));
        ...
        hierarchy.push_str("### 📋 Current Entry Details\n\n");
        hierarchy.push_str(&format!("- **Branch:** `{}`\n", current_entry.branch));
        hierarchy.push_str(&format!(
            "- **Commit:** `{}`\n",
            &current_entry.commit_hash[0..8]
        ));

        if let Some(pr_id) = &current_entry.pull_request_id {
            hierarchy.push_str(&format!("- **Pull Request:** #{pr_id}\n"));
        }

        // Add navigation help
        hierarchy.push_str("\n### 🧭 Navigation\n\n");
        hierarchy.push_str("- **Previous Entry:** ");

        if let Some(prev_index) = stack
            .entries
            .iter()
            .position(|e| e.id == current_entry.id)
            .and_then(|i| i.checked_sub(1))
        {
            let prev_entry = &stack.entries[prev_index];
            if let Some(pr_id) = &prev_entry.pull_request_id {
                // You would construct the actual Bitbucket URL here
                hierarchy.push_str(&format!("`{}` (PR #{})", prev_entry.branch, pr_id));
            } else {
                hierarchy.push_str(&format!("`{}` (not submitted)", prev_entry.branch));
            }
        } else {
            hierarchy.push_str(&format!("`{}` (base branch)", stack.base_branch));
        }

        hierarchy.push('\n');
        hierarchy.push_str("- **Next Entry:** ");

        if let Some(current_index) = stack.entries.iter().position(|e| e.id == current_entry.id) {
            if current_index + 1 < stack.entries.len() {
                let next_entry = &stack.entries[current_index + 1];
                if let Some(pr_id) = &next_entry.pull_request_id {
                    hierarchy.push_str(&format!("`{}` (PR #{})", next_entry.branch, pr_id));
                } else {
                    hierarchy.push_str(&format!("`{}` (not submitted)", next_entry.branch));
                }
            } else {
                hierarchy.push_str("*This is the top of the stack*");
            }
        } else {
            hierarchy.push_str("*Unknown*");
        }
